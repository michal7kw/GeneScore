...
def main():
    ...
    if all_sim_top:
        ...
        all_sim_save.to_csv(os.path.join(output_dir, 'scores_sim_all.csv'), index=False)
        print(f"Simulation scores saved to {os.path.join(output_dir, 'scores_sim_all.csv')}")

    if all_grn_combined:
        all_grn_save = pd.concat(all_grn_combined, ignore_index=True)
        all_grn_save.to_csv(os.path.join(output_dir, 'scores_grn_all_from_comb_run.csv'), index=False)
        print(f"GRN scores saved to {os.path.join(output_dir, 'scores_grn_all_from_comb_run.csv')}")
